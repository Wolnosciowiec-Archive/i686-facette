...
ENV GO_VERSION=1.8.3 NODE_VERSION=7.10.0 PREFIX=/usr BUILD_TAGS=builtin_assets GOPATH=/usr/lib/go GOBIN=/usr/lib/go/bin
...
ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/local/go/bin:/usr/local/node/bin:/usr/lib/go/bin

WORKDIR /
RUN git clone https://github.com/facette/facette
WORKDIR /facette

RUN go get github.com/jteeuwen/go-bindata && cd ${GOPATH}/src/github.com/jteeuwen/go-bindata/go-bindata && go install
